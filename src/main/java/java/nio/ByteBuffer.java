...
public class ByteBuffer extends Buffer implements Comparable<ByteBuffer> {
	...
	public char getChar() {
		...
		char res = string(
				def.js.String.fromCharCode((int) _data.getInt16(position(), _order == ByteOrder.LITTLE_ENDIAN)))
	...
	public char getChar(int from) {
		...
		return string(def.js.String.fromCharCode((int) _data.getInt16(from, _order == ByteOrder.LITTLE_ENDIAN)))
				.charAt(0); // TODO use charset
	}
	...
	private static ByteBuffer wrap(byte[] array, int offset, int length) {
		...
		return new ByteBuffer(buffer, true); // currently can not able to write back. This is why it is a read-only
												// buffer
	}
